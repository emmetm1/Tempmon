...
while True:
    try:
        ...
        TempData = Point("Temp").tag("location", "Downstairs").field("temp_f", temp_f).time(datetime.utcnow(), WritePrecision.NS)
        HumidityData = Point("Humidity").tag("location", "Downstairs").field("humidity", humidity).time(datetime.utcnow(), WritePrecision.NS)
